...
describe('VCheckbox', () => {
  ...
  it('отображает заголовок, когда он передан', () => {
    ...
    expect(wrapper.find('[data-test="label"]').text()).toBe(labelText)
  })
  it('НЕ отображает заголовок, когда он НЕ передан', () => {
    const wrapper = getWrapper()
    expect(wrapper.find('[data-test="label"]').exists()).toBeFalsy()
  })
  it('label имеет оформление required когда required', async () => {
    const wrapper = getWrapper({
      required: true,
      label: 'Название обязательного чекбокса',
    })
    const labelWrapper = wrapper.find('[data-test="label"]')
    // проверка класса полагается на детали реализации (название класса), так лучше не делать
    // но проверить сами стили ::after не получилось
    expect(labelWrapper.classes()).toContain('required')
  })
  it('label НЕ имеет оформление required когда НЕ required', async () => {
    const wrapper = getWrapper({
      label: 'Название обязательного чекбокса',
    })
    const labelWrapper = wrapper.find('[data-test="label"]')

    expect(labelWrapper.classes()).not.toContain('required')
  })
  ...
  it ('когда чекнули, происходит эмит modelValue', async () => {
    const wrapper = getWrapper()
    await wrapper.find('[data-test="checkbox-input"]').setValue()
    expect(wrapper.emitted()).toHaveProperty('update:modelValue')
  })
  it ('когда анчекнули, происходит эмит modelValue', async () => {
    const wrapper = getWrapper()
    await wrapper.find('[data-test="checkbox-input"]').setValue()
    await wrapper.find('[data-test="checkbox-input"]').setValue(false)
    expect(wrapper.emitted()).toHaveProperty('update:modelValue')
  ...
  it('эмитит ошибку валидации, когда чекнули и анчекнули при required', async () => {
    const wrapper = getWrapper({ required: true, touchId: '' })
    // должен измениться touchId, чтобы запустилась валидация
    const checkbox = wrapper.find('[data-test="checkbox-input"]')
  ...
  it ('не имеет красной рамки по умолчанию', async () => {
    const wrapper = getWrapper()
    expect(wrapper.get('[data-test="checkbox-input"]').classes('border-red-300')).toBe(false)
  })
  it('имеет красный бордер, когда анчекнут при required', async () => {
    const wrapper = getWrapper({ required: true })
    await wrapper.setProps({ touchId: getRandomUid() })
    expect(wrapper.get('[data-test="checkbox-input"]').classes()).toContain('border-red-300')
  })
  ...
  it('принимает слот для description', async () => {
    const testDescriptionClass = 'test-description-class'
    const description = `<div class="${testDescriptionClass}">test description</div>`
    const wrapper = getWrapper(
      {},
      {
        description,
    ...
    expect(wrapper.find(`.${testDescriptionClass}`).exists()).toBeTruthy()
